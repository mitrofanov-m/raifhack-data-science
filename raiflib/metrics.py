...
def deviation_metric_one_sample(y_true: typing.Union[float, int], y_pred: typing.Union[float, int], metric_value: bool=True) -> typing.Union[float, int]:
    """
    Реализация кастомной метрики для хакатона.

    :param y_true: float, реальная цена
    :param y_pred: float, предсказанная цена
    :return: typing.Union[float, int], значение метрики или класс метрики
    """
    deviation = (y_pred - y_true) / np.maximum(1e-8, y_true)
    if np.abs(deviation) <= THRESHOLD:
        return 0 if metric_value else 0
    elif deviation <= - 4 * THRESHOLD:
        return 9 * NEGATIVE_WEIGHT if metric_value else -2
    elif deviation < -THRESHOLD:
        return NEGATIVE_WEIGHT * ((deviation / THRESHOLD) + 1) ** 2 if metric_value else -1
    elif deviation < 4 * THRESHOLD:
        return ((deviation / THRESHOLD) - 1) ** 2 if metric_value else 1
    else:
        return 9 if metric_value else 2

def deviation_metric(y_true: np.array, y_pred: np.array, isLog: bool = False) -> float:
    if isLog:
