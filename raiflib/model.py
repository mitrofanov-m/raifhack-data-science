...
class CommercialsAppraiserModel():
    """
    ...
    def _find_corr_coefficient(self, X_manual: pd.DataFrame, y_manual: pd.Series):
        """Вычисление универсального корректирующего коэффициента. 
        Используется в случаях, когда в тренировочных данных отстутсвует регион, изспользуемый в тестовом наборе
        ...
        self._corr_coef = deviation

    def _find_corr_coefficients(self, X_manual: pd.DataFrame, y_manual: pd.Series):
        """Вычисление корректирующего коэффициента по регионам. 

        :param X_manual: pd.DataFrame с ручными оценками
        :param y_manual: pd.Series - цены ручника
        """
        predictions = self.pipeline.predict(X_manual)
        data = {'y_manual': y_manual,
                'predictions': predictions,
                'region': X_manual.region}
        y_df = pd.DataFrame(data, index=y_manual.index)
        y_df['deviation'] = (y_df.y_manual - y_df.predictions)/y_df.predictions
        self.corr_coef = y_df.groupby('region').agg({'deviation':'median'})
    ...
    def fit(self, X_offer: pd.DataFrame, y_offer: pd.Series,
        ...
        Затем вычисляется среднее отклонение между ручными оценками и предиктами для корректировки стоимости
        ...
        logger.info(X_offer.columns[67:71])
        self.pipeline.fit(X_offer, y_offer, model__feature_name=[f'{i}' for i in range(71)],model__categorical_feature=['67','68','69','70'])
        logger.info('Find corr coefficient')
        self._find_corr_coefficients(X_manual, y_manual)
    ...
    def predict(self, X: pd.DataFrame) -> np.array:
        """Предсказание модели 
        Предсказываем преобразованный таргет, затем конвертируем в обычную цену через обратное
        преобразование.
        ...
        if self.__is_fitted:
            ...
            y_df = pd.DataFrame(data, index=X.index)
            
            corrected_price = y_df.apply(
                lambda t: self.correction(t['predictions'], t['region']),
                axis=1
            ).tolist()
    ...
    def correction(self, prediction, region):
        try:
            return prediction * (1 + self.corr_coef.loc[region].deviation)
        except KeyError:
            return prediction * (1 + self._corr_coef)

    def save(self, path: str):
        """Сериализует модель в pickle.

        :param path: str, путь до директории с моделями
        """
        directory_path = os.getcwd()
        folder_name = os.path.basename(directory_path)
        path = path + datetime.now().strftime("%Y%m%d_%H:%M:%S_") + folder_name
